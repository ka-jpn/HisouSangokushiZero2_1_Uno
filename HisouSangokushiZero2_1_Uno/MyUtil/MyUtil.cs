...
public static class MyUtil {
  ...
  public static async Task<string?> GetFileString(string filePath) => File.Exists(filePath) ? await filePath.MyApplyF(async v => { using StreamReader sr = new(v); return await sr.ReadToEndAsync(); }) : null;
}
public static class 汎用拡張メソッド {
  ...
  public static T MyApplyF<T>(this T obj,Func<T,T> func,Predicate<T>? isInvoke = null) => (isInvoke?.Invoke(obj) != false ? func : MyUtil.Identity<T>()).Invoke(obj);
  ...
  public static T MyApplyA<T>(this T obj,Action<T> action,Predicate<T>? isInvoke = null) => obj.MyApplyF(v => { action(v); return v; },isInvoke);
