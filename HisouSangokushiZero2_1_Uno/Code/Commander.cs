...
namespace HisouSangokushiZero2_1_Uno.Code {
	internal static class Commander {
		private static Dictionary<PostHead,PersonType?> GetPersonMap(GameState game,ECountry country,ERole role) => Enum.GetValues<PostHead>().ToDictionary(v => v,v => Person.GetPostPerson(game,country,new(role,new(v)))?.Key);
		private static Dictionary<PostHead,PersonType?> GetAreaDefensePersonMap(GameState game,ECountry defense,EArea battleArea) => new([new(PostHead.main,Person.GetPostPerson(game,defense,new(ERole.defense,new(battleArea)))?.Key)]);
		...
		internal static decimal CommanderRank(GameState game,CommanderType commander,ERole role) => (commander.MainPerson?.MyApplyF(v => Person.CalcRank(game,v,role))??0)+(commander.SubPerson?.MyApplyF(v => Person.CalcRank(game,v,role)/2)??0);
	}
}
