...
internal static class Text {
  ...
  internal static string BattleDeathPersonText(ERole role,List<PersonId> deathPersons,ECountry? battleCountry,Lang lang) => Ja.BattleDeathPersonText(role,deathPersons,battleCountry);
  ...
  internal static string? WarDeathPersonText(ECountry? country,List<PersonId> deathPersons,Lang lang) => Ja.WarDeathPersonText(country,deathPersons);
  ...
  internal static string FallPlayerCapitalDeathPersonText(List<PersonId> deathPerson,Lang lang) => Ja.FallPlayerCapitalDeathPersonText(deathPerson);
  ...
  private static class Ja {
    ...
    internal static string BattleDeathPersonText(ERole role,List<PersonId> deathPersons,ECountry? battleCountry) => $"{(role == ERole.attack ? $"{CountryText(battleCountry)}領に侵攻" : $"{CountryText(battleCountry)}軍の侵攻を守備")}した{string.Join("と",deathPersons.Select(v => v.Value))}が退却できず戦死";
    ...
    internal static string? WarDeathPersonText(ECountry? country,List<PersonId> deathPersons) => deathPersons.Count != 0 ? $"{country}の{string.Join("と",deathPersons.Select(v => v.Value))}が戦死" : null;
    ...
    internal static string ChangeHasCountryText(ECountry attackCountry,ECountry? defenseCountry,EArea targetArea) => $"{CountryText(defenseCountry)}領の{targetArea}が{CountryText(attackCountry)}領に";
    ...
    internal static string AppendUpdateMaxAreaNumLog(int? updatedMaxAreaNum,ECountry? defenseCountry,EArea targetArea) => $"{CountryText(defenseCountry)}領の{targetArea}を攻略して最大領土数を{updatedMaxAreaNum}に更新";
    internal static string TurnHeadLogText(GameState game) => $"------------{Turn.GetCalendarText(game)}------------";
    internal static string FallPlayerCapitalText(EArea? capital) => $"首都の{capital}が陥落";
    internal static string FallPlayerCapitalDeathPersonText(List<PersonId> deathPerson) => $"首都の陥落により{string.Join("と",deathPerson.Select(v=>v.Value))}が死亡";
