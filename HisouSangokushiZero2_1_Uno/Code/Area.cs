...
internal static class Area {
  ...
  internal static List<EArea> CalcOrdDefenseAreas(GameState game,ECountry country) {
    ...
    static decimal ComputeAreaPressure(GameState game,KeyValuePair<EArea,AreaData> area) => ComputeAdjacentAnotherCountryAreas(game,area).Select(adjacent => AdjacentAreaPersonRank(game,adjacent) + 1).Sum();
    ...
    static List<EArea> ComputeAdjacentAnotherCountryAreas(GameState game,KeyValuePair<EArea,AreaData> area) => area.Value.Country?.MyApplyF(country => GetAdjacentAnotherCountryAreas(game,country,area.Key)) ?? [];
  }
  internal static List<EArea> CalcOrdAffairAreas(GameState game,ECountry country) {
    ...
    static decimal ComputeAreaRemAffairs(GameState game,KeyValuePair<EArea,AreaData> area) => area.Value.AffairParam.MyApplyF(v => v.AffairsMax - v.AffairNow);
  }
