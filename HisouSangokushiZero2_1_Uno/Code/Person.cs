...
namespace HisouSangokushiZero2_1_Uno.Code {
  internal static class Person {
    ...
    private static bool IsAppearPerson(GameState game,PersonParam personParam) => GetAppearYear(personParam) == Turn.GetYear(game) && Turn.GetInYear(game) == UIUtil.yearItems.Length / 2;
    ...
    private static bool IsNaturalDeathJudge(int year,int inYear,PersonParam personParam) => (personParam.DeathYear + 10 - year).MyApplyF(v => v <= 0 || MyRandom.RandomJudge(1 / (v * UIUtil.yearItems.Length - inYear) * (year - personParam.BirthYear) / 60));
