...
namespace HisouSangokushiZero2_1_Uno.Code {
  internal static class Person {
    ...
    private static bool IsNaturalDeathJudge(GameState game,PersonId person,int year,int inYear) => GetPersonData(game,person)?.MyApplyF(personData => (personData.DeathYear + 10 - year).MyApplyF(v => v <= 0 || MyRandom.RandomJudge(1 / (v * Enum.GetValues<YearItems>().Length - inYear) * (year - personData.BirthYear) / 60))) ?? false;
    ...
    internal static int GetAppearYear(ScenarioId? scenario,PersonId person) => scenario?.MyApplyF(ScenarioBase.GetScenarioData)?.PersonMap.GetValueOrDefault(person)?.MyApplyF(v => v.GameAppearYear ?? (v.BirthYear + majorityAge)) ?? 0;
