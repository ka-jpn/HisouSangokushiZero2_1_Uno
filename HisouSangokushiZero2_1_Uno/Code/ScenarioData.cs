	...
	internal static class ScenarioData {
		...
		internal static readonly Dictionary<Scenario,ScenarioInfo> scenarios = new([
          ...
          {ECountry.濊,new CountryInfo(100,1,Color.FromArgb(255,187,170,153),
            ...
            (game)=>$"{(Country.IsPerish(game,ECountry.呉)?"&#10004;":"&#10006;")} 呉が滅亡\n{(Country.IsPerish(game,ECountry.蜀漢)?"&#10004;":"&#10006;")} 蜀漢が滅亡\n{Area.GetChinaAreas(1).MyApplyF(v=>v.Length*(0.65-(Country.GetPerishFrom(game,ECountry.蜀漢)==ECountry.魏?0.1:0)-(Country.GetPerishFrom(game,ECountry.呉)==ECountry.魏?0.05:0))<=Country.HasAreaCount(game,ECountry.魏,v)?"&#10004;":"&#10006;")}中華領域の{65-(Country.GetPerishFrom(game,ECountry.蜀漢)==ECountry.魏?10:0)-(Country.GetPerishFrom(game,ECountry.呉)==ECountry.魏?5:0)}%以上領有{(Country.GetPerishFrom(game,ECountry.蜀漢)==ECountry.魏,Country.GetPerishFrom(game,ECountry.呉)==ECountry.魏) switch{(true,true)=>"(蜀漢と呉を滅亡させたので15%軽減)",(true,false)=>"(蜀漢を滅亡させたので10%軽減)",(false,true)=>"(呉を滅亡させたので5%軽減)",(false,false)=>"" }}",
            0,0)},
