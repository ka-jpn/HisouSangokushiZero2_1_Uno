...
static void UpdateCountryPostPersons(MainPage page,GameState game) {
  ...
  static StackPanel CreatePersonPostPanelElems(MainPage page,GameState game,ERole role) {
    return new StackPanel { BorderBrush = new SolidColorBrush(GetPostFrameColor(game,null)),BorderThickness = new(postFrameWidth) }.MySetChildren([.. Enumerable.Range(0,BaseData.capitalPieceRowNum).Select(row => GetPersonPostLinePanel(page,game,role,row,game.PersonMap.Where(v => v.Value.Country == game.PlayCountry).ToDictionary()))]);
...
static Color GetPostFrameColor(GameState game,EArea? area) => area != null && (game.NowScenario?.MyApplyF(ScenarioData.scenarios.GetValueOrDefault)?.ChinaAreas ?? []).Contains(area.Value) ? Color.FromArgb(150,100,100,30) : Color.FromArgb(150,0,0,0);
...
internal static class UI {
  internal static void ResizeLogMessageUI(MainPage page,GameState game) {
