...
internal class Ja:ILangText {
  ...
  string? ILangText.ScenarioCaptionText(ScenarioId? scenarioId) => scenarioId?.MyApplyF(ScenarioBase.GetScenarioData)?.MyApplyF(v=>$"シナリオ:{scenarioId.Value}({v.StartYear}年開始 {v.EndYear}年終了)");
